...
def grab_gene_to_right(sfeat, bed_table):
    ...
    right_gene_bed = "SELECT %(bed_table)s.* \
                       ...
                       %(bed_table)s.strand = '%(Strand)s' AND %(bed_table)s.start > %(end)d \
...
def grab_gene_to_left(sfeat, bed_table):
    sfeat['bed_table'] = bed_table
    left_gene_bed = "SELECT %(bed_table)s.* \
                       FROM %(bed_table)s \
                       WHERE %(bed_table)s.chr = '%(Chr)s' AND \
                       %(bed_table)s.strand = '%(Strand)s' AND %(bed_table)s.end < %(start)d \
...
def merge_tables(cursor):
    merge = "CREATE TABLE merge_genes AS(SELECT DISTINCT left_genes.sfeat, left_genes.accn as left_gene, right_genes.accn as right_gene,\
            ...
            right_genes.end as right_end, (right_genes.start - left_genes.end ) as sdiff, right_genes.chr as chr ,right_genes.strand as strand \
            FROM left_genes , right_genes \
            WHERE left_genes.sfeat = right_genes.sfeat AND left_genes.chr = right_genes.chr \
            AND left_genes.strand = right_genes.strand)"
...
def find_left_right_gene(syn_map1, bed_table, cursor):
 #   filter_bed_table(syn_map1 , bed_table_name , cursor)
    ...
    for sfeat in non_retained:
        right_gene = grab_gene_to_right(sfeat, bed_table)
        left_gene = grab_gene_to_left(sfeat, bed_table)
...
def create_region_table(bed_table_name , syn_map1, cursor):
    ...
    left_region_new = "CREATE TABLE left_region as(SELECT DISTINCT left_genes.* , %(bed_table_name)s.start , %(bed_table_name)s.end , %(bed_table_name)s.chr, %(bed_table_name)s.strand \
                       FROM (SELECT DISTINCT merge_genes.accn, merge_genes.sfeat , merge_genes.left_gene, \
    ...
    right_region_new = "CREATE TABLE right_region as(SELECT DISTINCT right_genes.* , %(bed_table_name)s.start , %(bed_table_name)s.end , %(bed_table_name)s.chr, %(bed_table_name)s.strand \
                      FROM (SELECT DISTINCT merge_genes.accn, merge_genes.sfeat , merge_genes.right_gene , %(syn_map1)s.qfeat as right_gene_qfeat\
...
def orintation(cursor):
    ...
    opp_strand = "CREATE TABLE opp_strand as(SELECT Region.accn, Region.sfeat, Region.Right_gene , Region.Right_gene_qfeat, Region.left_gene, Region.left_gene_qfeat, Region.Rend as start, region.Lstart as end , \
                                            (region.Lend - Region.Rstart) as diff,  Region.seqid , Region.strand \
                                            FROM Region \
                                            WHERE (region.Rstart < Region.Lend ))"
...
def create_final_table(cursor):
    ...
    create_table = "CREATE TABLE IF NOT EXISTS FINAL (accn INTEGER , sfeat Varchar(1000), sleft_gene Varchar(1000),\
                    ...
                    start INTEGER, end INTEGER, diff INTEGER, seqid Varchar(10), strand Varchar(10), orientation Varchar(100), url Varchar(10000))"
    cursor.execute(create_table)
    
    small_regions_same ="INSERT INTO FINAL (accn , sfeat, sleft_gene, sright_gene ,qleft_gene, qright_gene , start , end , diff, seqid, strand, orientation) \
                       ...
                       same_strand.seqid, same_strand.strand, 'same' FROM same_strand WHERE same_strand.diff < 100000"
    cursor.execute(small_regions_same)
    
    small_regions_opp = "INSERT INTO FINAL (accn , sfeat, sleft_gene, sright_gene , qleft_gene, qright_gene , start , end , diff, seqid, strand, orientation) \
                        ...
                        opp_strand.left_gene_qfeat ,opp_strand.right_gene_qfeat, opp_strand.start, opp_strand.end, opp_strand.diff, \
                         opp_strand.seqid, opp_strand.strand, 'opp' FROM opp_strand WHERE opp_strand.diff < 100000"
    cursor.execute(small_regions_opp)
    
    sgenes_info = "UPDATE FINAL, merge_genes SET sdiff = merge_genes.qdiff, sstart = merge_genes.left_end , send =  merge_genes.right_start \
